...
def key_exchange_client(keys, text, type):
    ...
    if type == "encrypt":
        ...
        try:
            ...
            # print(f"Plaintext alice setelah didekripsi: {decrypted_text}")
...
key_bob = "09182736CCDDAABB"

# =============================== CONNECT TO alice USING SOCKET ==============================================
...
e_alice = client_socket.recv(1024).decode()
e_alice = int(e_alice)

# Receive data from the client
n_alice = client_socket.recv(1024).decode()
n_alice = int(n_alice)
...
# =========================================  STEP 3 - FROM ALICE   =============================================================
Nb_from_bob = client_socket.recv(1024).decode()
Nb_from_bob = eval(Nb_from_bob)
...
if Nb_from_bob == Nb:
    ...
    type = "decrypt"
    plaintext = key_exchange_client(key_alice, chipertext_from_alice, type)
    print("plaintext from alice: ", plaintext)
	
else:
    print("Nb is not valid, youre not Alice")
    client_socket.close()
...
# =========================================  STEP 1 - TO ALICE   =============================================================
print("=========== Send to Alice ===========")
send_1 = encrypt_rsa(pair_1, e_alice, n_alice)
send_1 = str(send_1)
...
pair_2 = eval(pair_2)
print("ini pair step 2 dari alice: ", pair_2)
Na = pair_2[:6]
print("Na: ", Na)
Nb_from_alice = ''.join(pair_2[6:])

print("Nb_from_alice: ", Nb_from_alice)
print("Nb: ", Nb)
# validate Na
if Nb_from_alice == Nb:
    print("Nb is valid")
    ...
    # =========================================  STEP 3 - TO ALICE   =============================================================
    Na_encrypted = encrypt_rsa(Na, e_alice, n_alice)
    Na_encrypted = str(Na_encrypted)
    client_socket.send(Na_encrypted.encode())
    # =========================================  STEP 4 - TO ALICE   =============================================================
    print("Let's Talks with Alice")
    client_socket.send(key_bob.encode())
    plaintext = "AABB12345678CDEE"
    type = "encrypt"
    chipertext = key_exchange_client(key_bob, plaintext, type)
    print("plaintext for alice: ", plaintext)
    print("chipertext for alice: ", chipertext)
